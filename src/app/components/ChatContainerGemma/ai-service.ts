...
export class AIService {
  ...
  private readonly API_URL = "/api/generate";
  private readonly IMAGE_API_URL = "/api/generate-image";
  private readonly PDF_API_URL = "/api/generate-pdf";
  ...
  public async generatePDFResponse(prompt: string, pdfContent: string, history: Array<{role: "user" | "assistant", content: string}> = [], signal?: AbortSignal): Promise<Response> {
    try {
      const requestBody: PDFRequestBody = {
        prompt,
        pdfContent,
        history,
        options: {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || "Failed to generate PDF response");
      }
